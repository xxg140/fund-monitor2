...
function removeFund(code){
  ...
  showConfirm(`确定删除 ${name}？`,()=>{
    ...
    saveDcaPlanToStorage(code,null); saveFunds(); renderFunds(); if(expandedCode===code) closeFundDetailModal(); showToast('已删除','success');
  });
}
function clearAllFunds(){
  if(!funds.length)return;
  showConfirm(`清空所有 ${funds.length} 个基金？`,()=>{ funds=[]; setDcaAllPlans({}); saveFunds(true); renderFunds(); closeFundDetailModal(); showToast('已清空','success'); });
}
...
function _fmtOpMeta(op, opts) {
  ...
  if (op.type === 'convert') {
    ...
    var tl2 = isTargetView ? ('转换自 ' + (op.outName || op.outCode)) : ('转换至 ' + (op.inName || op.inCode));
    var detail;
    if (op.outNav && op.inNav) {
      detail = (isTargetView ? ('转入' + (op.inAmount||0).toFixed(2) + '元') : ('转出' + (op.outShares||0).toFixed(2) + '份')) + '@净值' + op.inNav.toFixed(4) + '→获' + (op.inShares||0).toFixed(2) + '份';
    } else {
      detail = '转出' + (op.outShares||0).toFixed(2) + '份';
    }
    var infoLine2 = (op.date||'').slice(2) + '·' + (op.timeType === 'before3' ? '3点前' : '3点后') + '·' + detail;
...
function _renderConvertModal() {
  ...
  var fundOpts = otherFunds.map(function(f){
    return '<option value="' + escapeHtml(f.code) + '">' + escapeHtml(f.name||f.code) + '</option>';
  }).join('');
  ...
  var html =
    '<div style="margin-bottom:12px;"><div style="font-size:var(--三类字体大小);color:var(--三类字体颜色);font-weight:500;">转出：' + safeHtml(fundName) + '</div><div style="font-size:var(--二类字体大小);color:var(--三类字体颜色);margin-top:2px;">' + escapeHtml(_convertState.code) + '</div></div>' +
     '<div class="下距10"><label class="t-12 字段标签">转入</label><input list="convertInFundList" id="convertInFund" class="form-input" style="width:100%;box-sizing:border-box;" placeholder="输入基金代码" value="' + escapeHtml(inputVal) + '" onchange="_onConvertInFundChange(this.value)"><datalist id="convertInFundList">' + fundOpts + '</datalist></div>' +
...
function _onConvertInFundChange(val) {
  var code = val.split(' - ')[0].trim();
  var fund = funds.find(function(f){return f.code===code});
  _convertState.inCode = code; _convertState.inName = fund ? (fund.name||fund.code) : '';
  _convertState.outNav = null; _convertState.outNavDate = null; _convertState.inNav = null; _convertState.inNavDate = null;
  _renderConvertModal();
  if (!fund && code) {
    lookupFundName(code, function(name){
      if (name && _convertState.inCode === code) { _convertState.inName = name; _renderConvertModal(); }
      var inp = document.getElementById('convertInFund');
      if (inp && _convertState.inCode === code) inp.value = code + ' - ' + (_convertState.inName || code);
    });
  }
}
...
function scheduleDcaTrades() {
  ...
  for (var code in plans) {
    ...
    while (cursor <= endDate) {
      ...
      if (dayOfWeek === 0 || dayOfWeek === 6) { cursor.setDate(cursor.getDate() + 1); continue; }
...
async function confirmSingleOp(fund, op) {
  ...
  if (op.type === 'convert') {
    ...
    if (op.outNav && op.inNav) {
      ...
      if (srcFund) { srcFund.shares = Math.max(0, (srcFund.shares||0) - op.outShares); if (srcFund.shares <= 0) { srcFund.cost = 0; srcFund.amount = 0; } else { var oldCost = Number(srcFund.cost)||0; srcFund.amount = r2(oldCost * srcFund.shares); } }
      var tgtFund = funds.find(function(f){return f.code===op.inCode});
      if (tgtFund) { var oldShare = tgtFund.shares||0; var oldAmount = tgtFund.amount||0; tgtFund.shares = oldShare + op.inShares; tgtFund.amount = r2(oldAmount + op.inAmount); tgtFund.cost = tgtFund.shares > 0 ? r2(tgtFund.amount / tgtFund.shares) : 0; }
      op.status = 'confirmed';
    }
    return;
  }
