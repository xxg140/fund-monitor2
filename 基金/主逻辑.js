...
const defaultColumns = [
  { field: 'name', title: '基金名称', width: 171, fr: 171, align: 'left', sortable: true, visible: true },
  { field: 'amount', title: '实时估值', width: 101, fr: 101, align: 'left', sortable: true, visible: true },
  { field: 'gszzl', title: '当日收益', width: 101, fr: 101, align: 'left', sortable: true, visible: true },
  { field: 'profit', title: '持有收益', width: 101, fr: 101, align: 'left', sortable: true, visible: true },
...
function showColSettings(){
  ...
  columns.forEach((col, i) => {
    ...
    html += `<div class="ts-row col-drag-row" data-colidx="${i}" onselectstart="return false" oncontextmenu="return false">
      ...
      <input type="checkbox" ${col.visible !== false ? 'checked' : ''} onchange="toggleColVisible(${i},this.checked)" style="accent-color:var(--强调色);flex-shrink:0;">
    </div>`;
  });
  body.innerHTML = html;
...
function renderGroupTabs() {
  ...
  html += `<div class="btn-main ${summaryActive}" onclick="switchToGroupSummary()">账户汇总</div>`;
  groups.forEach(g => {
    ...
    html += `<div class="btn-main ${active}" onclick="switchGroup('${escapedG}')">${escapeHtml(g)} (${count})</div>`;
  });
...
function renderGroupSummary() {
  ...
  if(summaryBar) {
    ...
    document.getElementById('sumLocked').className = 'summary-asset-value flat';
    ...
    document.getElementById('sumProfitToday').className = 'summary-today-value ' + gCc1;
    document.getElementById('sumProfitTodayRate').textContent = (todayRateVal>=0?'+':'-') + Math.abs(todayRateVal).toFixed(2) + '%';
    document.getElementById('sumProfitTodayRate').className = 'summary-today-rate-inline ' + gCc1Rate;
...
function renderGroupModal() {
  ...
  groups.forEach((g, idx) => {
    ...
    html += `<div class="group-list-item">
      ...
      ${isAll?'<span class="t-12">默认</span>':`<button class="btn-danger" onclick="deleteGroup(this.dataset.name)" data-name="${safeHtml(g)}">删除</button>`}
    </div>`;
  });
  html += `<div class="group-add-row">
    <input type="text" class="action-input 弹性1" id="newGroupName" placeholder="新分组名称" onclick="event.stopPropagation()">
  </div>`;
  body.innerHTML = html;
}
function addGroup() {
...
function switchImportTab(tab, btn){
  document.querySelectorAll('#importModal .btn-tab').forEach(t=>t.classList.remove('active'));
...
function showActionModal(code, type){
  ...
  let bodyHtml = `<div class="文字截断" style="margin-bottom:8px;font-size:14px;color:var(--弱化文字色);font-weight:500;">${escapeHtml(fundName)}</div>`;
  if(type === 'group'){
    ...
    const sharesInfoHtml = pendingReduce > 0
      ? `<div style="font-size:13px;color:var(--主文字色);font-weight:500;margin-bottom:4px;">当前持有 ${currentShares2} 份 · <span style="color:var(--强调色);">预扣 ${pendingReduce.toFixed(2)} 份</span></div>`
      : `<div style="font-size:13px;color:var(--主文字色);font-weight:500;margin-bottom:4px;">当前持有 ${currentShares2} 份</div>`;
    bodyHtml += `<div style="display:flex;flex-direction:column;gap:10px;">
      ...
      <div class="内距10" id="tradePanel_${code}">
        ...
        <div id="tradeReduceFields_${code}" style="display:none;">
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;"><input type="number" id="reduceShares_${code}" placeholder="卖出份额" step="0.01" min="0" ${inputCls} style="flex:1;" oninput="this.parentElement.nextElementSibling.querySelectorAll('.btn-tab').forEach(b=>b.classList.remove('active'))"></div>
          <div style="display:flex;gap:6px;margin-bottom:6px;flex-wrap:wrap;"><button class="btn-tab 字号11" onclick="setReduceShares('${code}',0.25,this)">1/4</button><button class="btn-tab 字号11" onclick="setReduceShares('${code}',1/3,this)">1/3</button><button class="btn-tab 字号11" onclick="setReduceShares('${code}',0.5,this)">1/2</button><button class="btn-tab active 字号11" onclick="setReduceShares('${code}',1,this)">全部</button></div>
          ${sharesInfoHtml}
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:6px;"><input type="date" id="addDate_${code}" value="${today2}" ${inputCls} style="width:140px;"><div style="display:flex;gap:4px;" id="tradeTimeBtns2_${code}"><button class="btn-tab ${(_tradeTimeType[code]||'before3')==='before3'?'active':''}" onclick="setTradeTimeType('${code}','before3')" id="tradeBefore3_${code}">3点前</button><button class="btn-tab ${(_tradeTimeType[code]||'before3')==='after3'?'active':''}" onclick="setTradeTimeType('${code}','after3')" id="tradeAfter3_${code}">3点后</button></div></div>
        <button class="btn-action" onclick="(function(){var m=document.getElementById('tradePanel_${code}').dataset.mode||'add';if(m==='add'){submitAddTrade('${code}')}else{document.getElementById('reduceDate_${code}')||(function(){var d=document.createElement('input');d.type='hidden';d.id='reduceDate_${code}';document.body.appendChild(d)})();document.getElementById('reduceDate_${code}').value=document.getElementById('addDate_${code}').value;submitReduceTrade('${code}')}})()">确认修改</button>
      </div>
    </div>`;
  }
...
function _renderOpItem(op, opts) {
  ...
  const fundLabel = opts.showFundName ? `<span style="font-size:13px;font-weight:600;color:var(--主文字色);">${safeHtml(op.fundName)}</span>` : '';
  const badgeOrInline = opts.badge !== false ? m.badge : `<span style="color:${m.sc};font-size:11px;font-weight:600;">${m.sl}</span>`;
  const cancelBtn = m.canCancel ? `<button class="btn-danger" style="font-size:10px;" data-op-action="cancel" data-op-code="${escapeHtml(op.fundCode||code)}" data-op-id="${escapeHtml(op.id)}">撤回</button>` : '';
  const deleteBtn = m.canDelete ? `<button class="btn-danger" style="font-size:10px;" data-op-action="delete" data-op-code="${escapeHtml(op.fundCode||code)}" data-op-id="${escapeHtml(op.id)}">删除</button>` : '';
  return `<div style="padding:10px 0;border-bottom:1px solid var(--浅色边框);">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
      <div style="flex:1;min-width:0;">
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">${fundLabel}<span style="font-size:${opts.showFundName?'12':'13'}px;font-weight:500;color:${opts.showFundName?'var(--次文字色)':'var(--主文字色)'};">${m.tl} ${safeHtml(op.amount)}${m.unit}</span></div>
        <div style="font-size:11px;color:var(--弱化文字色);margin-top:2px;">${m.infoLine}</div>
      </div>
...
function renderOpListModal(code) {
  ...
  let html = '';
  if (pendingCount > 0) html += `<div style="font-size:11px;color:var(--强调色);font-weight:600;margin-bottom:8px;">有 ${pendingCount} 笔待确认操作</div>`;
  if (!ops.length) { html += '<div class="左对齐" style="font-size:13px;color:var(--弱化文字色);padding:24px;">暂无操作记录</div>'; container.innerHTML = html; return; }
...
function switchDetailTab(code, tabName, btn) {
  ...
  tabs.forEach(t => { t.style.borderBottomColor = 'transparent'; t.style.color = 'var(--弱化文字色)'; t.classList.remove('active'); });
  btn.style.borderBottomColor = 'transparent';
  btn.style.color = 'var(--强调色)';
  btn.classList.add('active');
...
async function _loadHoldingsPanel(code) {
  ...
  html += `<div class="holding-item" style="font-size:10px;color:var(--弱化文字色);font-weight:500;padding:8px;border-bottom:1px solid var(--默认边框);">
    <span class="序号列">#</span>
    <span class="h-name" style="color:var(--弱化文字色);font-weight:500;">股票名称</span>
  ...
  holdings.forEach((h, i) => {
    ...
    html += `<div class="holding-item">
      <span class="t-10" class="序号列">${i + 1}</span>
...
function _renderHistoryMiniTable(container, rows) {
  ...
  if (rows.length > 15) {
    html += `<div style="margin-top:8px;display:flex;justify-content:center;">
      <button type="button" class="btn-icon t-12"
...
function _showHistoryModal(btn) {
  ...
  overlay.innerHTML = `<div class="modal-box" style="max-height:80vh;display:flex;flex-direction:column;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px 10px;">
      <span style="font-weight:600;font-size:15px;color:var(--主文字色);">历史净值</span>
      <button type="button" class="btn-icon" style="font-size:18px;color:var(--弱化文字色);padding:4px 8px;line-height:1;"
...
async function switchChartPeriod(period,btn){
  ...
  btn.closest('.chart-tabs')?.querySelectorAll('.btn-tab').forEach(t=>t.classList.remove('active'));
  btn.classList.add('active');
  const fund=funds.find(f=>f.code===code);
...
function renderIndices(){
  ...
  if(!selectedIndices.length){
    container.innerHTML='<div class="index-card add-card 居中对齐" onclick="openIndexModal()" style="grid-column:1/-1;color:var(--弱化文字色);font-size:12px;padding:14px;">+ 点击选择指数</div>';
    return;
  }
  let html='';
  selectedIndices.forEach(code=>{
...
function openIndexDetail(code) {
  ...
  var tabsHtml = _indexTabOrder.map(function(p) {
    return '<button class="btn-tab ' + (p === _indexDetailPeriod ? 'active' : '') + '" onclick="switchIndexPeriod(\'' + p + '\')">' + (periodNames[p] || p) + '</button>';
  }).join('') + '<button class="btn-icon" onclick="toggleTabSettings(event)" title="自定义排序">' + ICON_GEAR + '</button>';
  // 从缓存获取数据，直接填充
  ...
  var priceColor = cls === 'up' ? 'var(--上涨色)' : (cls === 'down' ? 'var(--下跌色)' : 'var(--主文字色)');
  var chgColor = cls === 'up' ? 'var(--上涨色)' : (cls === 'down' ? 'var(--下跌色)' : 'var(--弱化文字色)');
  ...
  body.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px 6px;margin-bottom:8px;">'
    ...
    + '<div id="indexChartInfo" style="font-size:12px;background:transparent;border:1px solid var(--悬停边框);border-radius:12px;min-height:34px;box-sizing:border-box;padding:6px 10px;margin-bottom:4px;display:flex;align-items:center;gap:0;font-variant-numeric:tabular-nums;">'
    ...
    + '<div id="hkChartOverlay" style="position:absolute;top:0;left:0;width:100%;height:100%;display:none;align-items:center;justify-content:center;z-index:2;pointer-events:none;"><span style="color:var(--次文字色);font-size:13px;font-weight:500;background:var(--次级区域背景);padding:6px 16px;border-radius:6px;">分时/五日暂不支持港股</span></div>'
    + '</div></div>';
...
function renderIndexStatic(info) {
  ...
  if (!data) { body.innerHTML = '<div class="居中对齐" style="padding:40px 0;color:var(--弱化文字色);">暂无数据</div>'; return; }
  ...
  var chartMsg = isUs ? '该指数暂不支持K线图' : '图表数据加载失败，请检查网络后重试';
    var retryBtn = isUs ? '' : '<button class="btn-action 字号12" onclick="retryIndexChart(\'' + code + '\')">重新加载</button>';
  body.innerHTML = '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px;">'
    + '<div class="统计卡"><div class="t-10">最新价</div><div style="font-size:18px;font-weight:700;color:' + (cls==='up'?'var(--上涨色)':cls==='down'?'var(--下跌色)':'var(--主文字色)') + '">' + data.price.toFixed(2) + '</div></div>'
    + '<div class="统计卡"><div class="t-10">涨跌幅</div><div style="font-size:14px;font-weight:600;color:' + (cls==='up'?'var(--上涨色)':cls==='down'?'var(--下跌色)':'var(--弱化文字色)') + '">' + sign + Math.abs(chg).toFixed(2) + '%</div></div>'
    + '<div class="统计卡"><div class="t-10">涨跌额</div><div style="font-size:14px;font-weight:600;color:' + (cls==='up'?'var(--上涨色)':cls==='down'?'var(--下跌色)':'var(--弱化文字色)') + '">' + pointChgSign + Math.abs(pointChg).toFixed(2) + '</div></div>'
    + '</div>'
    + '<div class="居中对齐" style="padding:30px 0;">'
    + '<div style="color:var(--弱化文字色);font-size:12px;margin-bottom:10px;">' + chartMsg + '</div>'
    + retryBtn
    + '</div>';
}
...
function renderIndexChart(info, rawData, period, apiPreClose, trend5Base) {
  ...
  // 构建界面
  const priceColor = cls === 'up' ? 'var(--上涨色)' : (cls === 'down' ? 'var(--下跌色)' : 'var(--主文字色)');
  const chgColor = cls === 'up' ? 'var(--上涨色)' : (cls === 'down' ? 'var(--下跌色)' : 'var(--弱化文字色)');
  // 更新信息栏（不重建 HTML）
  ...
  if (_icHigh) { _icHigh.textContent = todayHigh.toFixed(2); _icHigh.style.color = 'var(--上涨色)'; }
  var _icLow = document.getElementById('icLow');
  if (_icLow) { _icLow.textContent = todayLow.toFixed(2); _icLow.style.color = 'var(--下跌色)'; }
  if (!isTrend) {
  ...
  // 更新 tabs 激活状态
  document.querySelectorAll('#indexDetailTabs .btn-tab').forEach(function(t) { t.classList.remove('active'); });
  var activeTab = document.querySelector('#indexDetailTabs .btn-tab[onclick*="\'' + period + '\'"]');
...
function switchIndexPeriod(period) {
  ...
  if (!info) return;
  // 更新 tab 激活状态
  document.querySelectorAll('#indexDetailTabs .btn-tab').forEach(t => t.classList.remove('active'));
  const activeTab = document.querySelector(`#indexDetailTabs .btn-tab[onclick*="'${period}'"]`);
  if (activeTab) activeTab.classList.add('active');
  // 旧图变淡，提示正在加载新数据
...
function applyTabOrder() {
  ...
  if (container && _indexDetailCode) {
    ...
    container.innerHTML = _indexTabOrder.map(p =>
      `<button class="btn-tab ${p===period?'active':''}" onclick="switchIndexPeriod('${p}')">${periodNames[p]}</button>`
    ).join('') + '<button class="btn-icon" onclick="toggleTabSettings(event)" title="自定义排序">' + ICON_GEAR + '</button>';
  }
}
...
function drawIndexChart(canvas, points, isTrend, color, preClose, period) {
  ...
  if (dEl) {
    if (isTrend) {
      ...
      $ic(pEl, pct.txt || '涨幅 --', pct.clr || 'var(--弱化文字色)');
    } else {
      if (oEl && oEl.parentElement) oEl.parentElement.style.display = '';
      ...
      $ic(pEl, pct2.txt || '涨幅 --', pct2.clr || 'var(--弱化文字色)');
    }
  }
...
function _updateICInfo(points, idx, isTrend, period, preClose, color) {
  ...
  if (isTrend) {
    ...
    $ic(cEl, '收 ' + p.price.toFixed(2), trIsUp ? 'var(--上涨色)' : 'var(--下跌色)');
    if (oEl && oEl.parentElement) oEl.parentElement.style.display = '';
...
function renderFunds(){
  ...
  columns.forEach((col, idx) => {
    ...
    const gearIcon = col.field === 'name' ? `<span class="btn-icon col-gear" onclick="event.stopPropagation();showColSettings()" title="列设置">${ICON_GEAR}</span>` : '';
    const content = col.field === 'name' ? `${col.title}${sortArrow}${gearIcon}` : `${sortArrow}${col.title}${gearIcon}`;
    html += `<div class="lh-cell ${sortClass} ${activeClass}" data-field="${col.field}" onclick="toggleSort('${col.field}')">${content}</div>`;
  });
  html += '</div>';
  var groupTotalMV = 0;
  ...
  displayFunds.forEach(fund=>{
    ...
    columns.forEach(col => {
      ...
      if (col.field === 'name') {
        ...
        colsHtml += `<div class="fm-name" data-code="${fund.code}" style="padding-left:13px;">
        ...
        colsHtml += `<div class="fm-rate" data-code="${fund.code}">
          ...
          <div class="fm-rate-sector">${safeHtml(sectorName)}</div>
        </div>`;
  ...
  document.getElementById('sumProfitToday').className='summary-today-value '+cc1;
  document.getElementById('sumProfitTodayRate').textContent=(todayRateVal2>=0?'+':'-')+Math.abs(todayRateVal2).toFixed(2)+'%';
  document.getElementById('sumProfitTodayRate').className='summary-today-rate-inline '+cc1Rate2;
  var sumTag = document.getElementById('sumUpdatedTag');
...
function _fmtOpMeta(op) {
  ...
  const sc = STATUS_COLORS[op.status] || 'var(--弱化文字色)';
...
function calcPct(currVal, prevVal) {
  ...
  if (absVal < 0.005) return { txt: '涨幅 0.00%', clr: 'var(--弱化文字色)' };
  const sign = chgVal > 0 ? '+' : '-';
  return { txt: `涨幅 ${sign}${absVal.toFixed(2)}%`, clr: chgVal > 0 ? 'var(--上涨色)' : 'var(--下跌色)' };
}
...
function openFundDetailModal(code){
  ...
  const periodBtns = [1,3,6,12,36].map(p => {
    ...
    return `<button class="btn-tab${active}" onclick="switchChartPeriod(${p},this);event.stopPropagation()">${labels[p]}</button>`;
  }).join('');
  const codeHtml = fund.code ? `<div style="font-size:14px;color:var(--次文字色);font-weight:500;margin-top:2px;">${fund.code}</div>` : '';
  document.getElementById('fundDetailTitle').innerHTML = `<div style="display:flex;flex-direction:column;"><span style="display:inline-flex;align-items:center;gap:6px;">${safeHtml(fund.name)||('基金'+safeHtml(fund.code))} <button class="btn-danger" onclick="event.stopPropagation();removeFund('${escapeHtml(fund.code)}')">删除</button></span>${codeHtml}</div>`;
  ...
  const html = `<div class="detail-inner">
    <div class="detail-debug" style="font-size:12px;color:var(--次文字色);padding:4px 8px;">
    ...
    <div style="padding:8px 8px 4px;">
      <div style="display:flex;align-items:stretch;background:transparent;border-radius:12px;overflow:hidden;border:1px solid var(--悬停边框);">
        <button onclick="event.stopPropagation();showActionModal('${escapeHtml(fund.code)}','group')" class="详情操作按钮"><svg class="icon-svg" viewBox="0 0 24 24"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg> 修改分组</button>
        <span class="竖分隔"></span>
        <button onclick="event.stopPropagation();showActionModal('${escapeHtml(fund.code)}','holding')" class="详情操作按钮"><svg class="icon-svg" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> 修改持仓</button>
        <span class="竖分隔"></span>
        <button onclick="event.stopPropagation();showActionModal('${escapeHtml(fund.code)}','oplist')" class="详情操作按钮"><svg class="icon-svg" viewBox="0 0 24 24"><path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/></svg> 交易记录</button>
      </div>
    </div>
    <div class="detail-tabs" id="detailTabs_${fund.code}" style="margin:4px 0;gap:0;position:relative;">
      <button class="btn-icon detail-tab active" style="color:var(--强调色);" onclick="switchDetailTab('${escapeHtml(fund.code)}','trend',this);event.stopPropagation()">业绩走势</button>
      <button class="btn-icon detail-tab" style="color:var(--弱化文字色);" onclick="switchDetailTab('${escapeHtml(fund.code)}','holdings',this);event.stopPropagation()">前10持仓</button>
      <button class="btn-icon detail-tab" style="color:var(--弱化文字色);" onclick="switchDetailTab('${escapeHtml(fund.code)}','history',this);event.stopPropagation()">历史净值</button>
      <div class="tab-indicator" id="tabIndicator_${fund.code}" style="position:absolute;bottom:0;height:2px;background:var(--强调色);transition:left 0.3s ease,width 0.3s ease;"></div>
    </div>
  ...
  setTimeout(()=>{
    ...
    // 初始化滑动下划线位置
    const indicator = document.getElementById('tabIndicator_' + code);
    const firstTab = document.querySelector('#detailTabs_' + code + ' .detail-tab.active');
...
function toggleAutoRefresh(){
  ...
  if(isAutoRefresh){
    if(m)m.innerHTML='<svg class="icon-svg 图标间距" viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> 停止自动刷新';
    refreshAll(); autoRefreshTimer=setInterval(()=>refreshAll(),AUTO_REFRESH_INTERVAL);
    showToast('自动刷新已开启（60秒）','success');
  }else{
    if(m)m.innerHTML='<svg class="icon-svg 图标间距" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> 自动刷新';
...
// 初始化指数栏滑动指示器
(function() {
  var b = document.querySelector('.index-bar-tab.active');
  if (b) { var i = b.parentElement.querySelector('.index-bar-indicator'); if (i) { i.style.left = b.offsetLeft + 'px'; i.style.width = b.offsetWidth + 'px'; } }
})();
// 按钮不获取焦点，避免浏览器默认聚焦样式残留（在 pointerdown 时 blur，不干扰 :active 动画）
document.addEventListener('pointerdown', e => { if (e.target.matches('.btn-main,.btn-action,.btn-tab,.btn-danger')) e.target.blur(); });
...
function renderTradeOpList(code, containerId) {
  ...
  let html = '<div style="font-size:11px;font-weight:600;color:var(--弱化文字色);margin-bottom:6px;">操作记录</div>';
  const sorted = [...ops].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
...
function renderGlobalOpList() {
  ...
  let html = '';
  if (pendingCount > 0) html += `<div style="font-size:12px;color:var(--强调色);font-weight:600;margin-bottom:10px;background:var(--强调浅色);padding:8px 12px;border-radius:12px;">有 ${pendingCount} 笔待确认操作，将在下次刷新时自动确认</div>`;
  if (!allOps.length) { html += '<div class="居中对齐" style="font-size:14px;color:var(--弱化文字色);padding:40px;">暂无操作记录</div>'; container.innerHTML = html; return; }
...
function switchIndexBarTab(tab, btn) {
  ...
  btn.classList.add('active');
  var indicatorEl = btn.parentElement.querySelector('.index-bar-indicator');
  if (indicatorEl) {
...
function loadSectorData(type) {
  ...
  if (el) el.innerHTML = '<div class="居中对齐" class="状态提示">加载中...</div>';
  ...
  function fetchPage(page, onFirstPage) {
    ...
    window[cbName] = function(d) {
      ...
      if (!d || !d.data || !d.data.diff) {
        failed = true;
        if (el) el.innerHTML = '<div class="居中对齐" class="状态提示">暂无数据</div>';
        return;
      }
      var raw = d.data.diff;
    ...
    s.onerror = function() {
      ...
      if (!failed && el) {
        failed = true;
        el.innerHTML = '<div class="居中对齐" class="错误提示">加载失败，请检查网络</div>';
      }
    };
    ...
    setTimeout(function() {
      ...
      if (!failed && el) {
        failed = true;
        el.innerHTML = '<div class="居中对齐" class="错误提示">请求超时，请稍后重试</div>';
      }
    }, 10000);
...
function openSectorPanel() {
  ...
  // 重置到单个输入 tab
  switchSectorTab('single', document.querySelector('.sector-tabs .btn-tab'));
  document.getElementById('sectorBatchInput').value = '';
  if (!funds.length) {
    body.innerHTML = '<div class="居中对齐" style="padding:30px;color:var(--弱化文字色);">暂无基金</div>';
    ...
    sorted.forEach(f => {
      ...
      html += `<div style="display:flex;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid var(--浅色边框);">
        <div class="文字截断" style="flex:1;min-width:0;font-size:13px;font-weight:600;color:var(--主文字色);" title="${safeHtml(name)}">${safeHtml(name)}</div>
...
function switchSectorTab(tab, btn) {
  document.querySelectorAll('.sector-tabs .btn-tab').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  // 切换内容区
  const body = document.getElementById('sectorBody');
  const batchBody = document.getElementById('sectorBatchBody');
...
function saveSectors() {
  // 判断当前激活的是哪个 tab
  const activeTab = document.querySelector('.sector-tabs .btn-tab.active');
...
function editFundSectorInline(code) {
  ...
  input.type = 'text';
  input.value = oldVal;
  input.style.cssText = 'width:100%;font-size:12px;font-weight:500;color:var(--主文字色);background:var(--卡片背景);border:1px solid var(--强调色);border-radius:6px;padding:2px 6px;box-sizing:border-box;';
  el.innerHTML = '';
